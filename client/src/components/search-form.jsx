...
import {
  selectCheckInDate1,
  selectCheckOutDate1
} from "../redux/booking/booking.selectors.js";
      ...
      axios
        ...
        .then(data => {
          this.setState({ title: data.title }, () =>
            this.setState({ cleaningfee: data.cleaning_fee }, () =>
              this.setState({ sleepcapacity: data.sleep_capacity }, () =>
                this.setState({ reviewnumber: data.review_number }, () =>
                  this.setState({ reviewoverview: data.review_overview }, () =>
                    this.setState(
                      { owner: data.owner },
                      this.setState({ rating: data.rating }, () =>
                        this.setState({ usstate: data.state }, () =>
                          this.setState({ city: data.city }, () =>
                            this.setState({ pic: data.pic }, () => null)
                          )
                        )
                      )
                    )
    ...
    return (
      <div className="al-search-form-container">
        ...
        <CheckInInput
          ...
          handleCheckInSelect={this.handleCheckInSelect}
          openCalendar={this.openCalendar}
        />
        <CheckOutInput
          check_out={this.state.check_out}
          handleCheckOutSelect={this.handleCheckOutSelect}
          openCalendar={this.openCalendar}
        />
        ...
        <div className="al-search-dropdown-container">
          {this.state.term.length ? (
            <SearchDropdown
              ...
              searchlistings={this.state.searchlistings.slice(0, 10)}
            />
          ) : null}
        </div>
      </div>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    setSleepCapacity: sleepCapacity =>
      dispatch(setSleepCapacity(sleepCapacity)),
    setReviewNumber: reviewNumber => dispatch(setReviewNumber(reviewNumber)),
    setReviewOverview: reviewOverview =>
      dispatch(setReviewOverview(reviewOverview)),
